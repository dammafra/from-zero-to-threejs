...
const updateGrid = (elapsedTime, delta) => {
  if (shrinking) {
    const currentScale = targetTile.scale.x
    targetTile.scale.setScalar(Math.max(0, currentScale - 1.5 * delta))
    shrinking = currentScale > 0
    growing = !shrinking
    return
  }
  ...
  if (growing) {
    const currentScale = targetTile.scale.x
    targetTile.scale.setScalar(Math.min(0.92, currentScale + 1.5 * delta))
    growing = currentScale < 0.92
    targetTile = growing ? targetTile : undefined
    return
  }

  if (!targetTile) {
    const randomIndex = Math.floor(Math.random() * grid.children.length)
    targetTile = grid.children.at(randomIndex)
    return
  }

  switch (Math.floor(elapsedTime % 5)) {
    case 1:
      targetTile.material.color.set('limegreen')
      break
    case 2:
      targetTile.material.color.set('orange')
      break
    case 3:
      targetTile.material.color.set('red')
      break
    case 4:
      targetTile.material.color.set('dodgerblue')
      shrinking = true
      break
  }
}
...
const tick = () => {
  ...
  // Update character and grid
  updateCharachter(elapsedTime, delta)
  updateGrid(elapsedTime, delta)
